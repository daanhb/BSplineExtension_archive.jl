...
A = [];
    push!(A,@pgf {xmode="log",ymode="log",legend_pos="north west",xlabel="N"}) # hide
    ...
    push!(A,@pgf {xmode="log",ymode="log",xlabel="N"}) # hide
    for (j,d) in enumerate(ds) # hide
...
i = 1
    ...
    for (j,d) in enumerate(ds), (k,N) in enumerate(Ns1) #hide
        P = ExtensionFramePlatform(NdEpsBSplinePlatform((d,d)), (0.0..0.5)^2) #hide
        _,timings[i,j,k],_ = @timed Fun(f, P, (N,N);REG=BSplineExtension.BSplineExtensionSolver, crop=true, crop_tol=1e-10, verbose=verbose) #hide
    end #hide
    i = 2
    for (j,d) in enumerate(ds), (k,N) in enumerate(Ns1) #hide
        P = ExtensionFramePlatform(NdCDBSplinePlatform((d,d)), (0.0..0.5)^2) #hide
...
for (j,d) in enumerate(ds), (k,N) in enumerate(Ns1) #hide
    ...
    _,timings[i,j,k],_ = @timed Fun(f, P, (N,N,N);REG=BSplineExtension.BSplineExtensionSolver, crop=true, crop_tol=1e-10, verbose=verbose) #hide
    errors[i,j,k] = abserror(f, F) #hide
end #hide
i = 2
for (j,d) in enumerate(ds), (k,N) in enumerate(Ns1) #hide
    ...
    errors[i,j,k] = abserror(f, F) #hide
    _,timings[i,j,k],_ = @timed Fun(f, P, (N,N,N);REG=BSplineExtension.BSplineExtensionSolver, crop=true, sparse=true, crop_tol=1e-10, verbose=verbose) #hide
end #hide
