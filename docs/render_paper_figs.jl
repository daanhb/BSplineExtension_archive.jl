...
i = 3
    for (j,d) in enumerate(ds), (k,N) in enumerate(Ns1) #hide
        @show d, N
        P = ExtensionFramePlatform(NdCDBSplinePlatform((d,d,d)), (0.0..0.5)^3) #hide
        F,_ = @timed Fun(f, P, (N,N,N);solverstyle=DirectStyle(), directsolver=sparseQR_solver,verbose=verbose) #hide
        errors[i,j,k] = abserror(f, F) #hide
        _,timings[i,j,k],_ = @timed Fun(f, P, (N,N,N);solverstyle=DirectStyle(), directsolver=sparseQR_solver) #hide
    end #hide
...
P = @pgf PGFPlotsX.GroupPlot({ymin=0,group_style={group_size="3 by 1",},}, # hide
    A...) # hide
DocumentPGFPlots.savefigs(joinpath(imgdir,"3dsparsedirecterrorpaper"),P;all=all) #
...
P = @pgf PGFPlotsX.GroupPlot({ymin=0,group_style={group_size="3 by 1",},}, # hide
    A...) # hide
DocumentPGFPlots.savefigs(joinpath(imgdir,"3dsparsedirecttimingspaper"),P;all=all)
...
DocumentPGFPlots.savefigs(joinpath(imgdir,"3dsparsedirecterrorpaper"),P;render=false)
DocumentPGFPlots.savefigs(joinpath(imgdir,"3dsparsedirecttimingspaper"),P;render=false)
